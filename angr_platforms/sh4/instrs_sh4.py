...
"""
PyVex instruction lifter for SH4
Author: AO (bob123456678)
Based on instrs in: http://www.shared-ptr.com/sh_insns.html
and code by pwnslinger / angr_platforms
Supports most non-floating point instructions
...
class SH4Instruction(Instruction):
	...
	TODO - maybe this should go in Instruction, since this could be useful
	for other architectures
	
	@param cond - a logical expression consisting of VexValues
		(if your expression only has constants, don't use this method!)
	@param valiftrue  - the VexValue to put in reg if cond == true
	@param validfalse - the VexValue to put in reg if cond != true
	@param reg - string representation of the register from the arch	
	"""
	...
	def compute_result(self, *args):
		...
		if ArchSH4.DELAYED_DEST_PC is not None and not ArchSH4.DELAYED_SET:
			...
			if isinstance(ArchSH4.DELAYED_DEST_PC, list):
				...
				# Do something (potentially) fancy
	...
	def resolve_one_reg(self, int_code, float=False):
		...
		# TODO: float instruction support is virtually nonexisted at the moment
		if float:
